...
abstract class AbstractNegotiator {
    ...
    public function getOrderedElements($header) {
        if (!$header) {
            throw new Exception\InvalidArgument('The header string should not be empty.');
        }
    ...
    private function parseHeader($header) {
        ...
        if (!$res) {
            throw new Exception\InvalidHeader(sprintf('Failed to parse accept header: "%s"', $header));
        }
